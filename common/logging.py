...
def configure_app_logger(debug=False, flaskapp=None):
    ...
    if debug:
        ...
        log_formatter = logging.Formatter(fmt='%(asctime)s : %(levelname)-8s %(message)s',\
                                          datefmt='%d/%b/%y %I:%M:%S%p')
    if configuration.config_name == 'testing_config': log_level = logging.CRITICAL
    ...
    if flaskapp:
        for app_handler in flaskapp.logger.handlers:
            flaskapp.logger.removeHandler(app_handler)
        flaskapp.logger.addHandler(file_handler)
        flaskapp.logger.addHandler(console_handler)
