...
def handle_http_error(error, headers=None):
    ...
    applogger.error("error on '{m} {r}' - {ec}, {rt}\nfrom-ip='{rip}', user-agent='{uas}'\n{rm}"\
                    .format(m=request.method, r=request.full_path, rip=utils.get_request_ip(),\
                            uas=request.user_agent.string, ec=error.status_code, rt=error.error_type,\
                            rm=pprint.pformat(response_dict)))
    return response
