...
class APIBase(MethodViewBase):
    #copied from https://py-jsonapi.readthedocs.io/en/latest/_modules/jsonapi/base/request.html#Request
    @property
    def input_filters(self):
        ...
        *   `modeq`,       modulo equality (attr % mod_value == 0)

        /api/user/1?filter[name]=endswith:"Simpson"
        /api/user/1?filter[parents.name]=endswith:"Home"
        /api/user/1?filter[name]=in:["Homer Simpson", "Darth Vader"]&filter[age]=eq:101
        /api/user/1?filter[email]=startswith:"lisa"&filter[age]=lt:20
    ...
    def select_fields(self, raw_response_data, exclude=None, many=False):
        ...
        if fields: return _fetch(self.response_schema, raw_response_data, many, exclude, only=fields.split(','))
        else:      return _fetch(self.response_schema, raw_response_data, many, exclude)
    ...
    def get_parsed_request_data(self):
        ...
        if request.json:
            try:    query_string_dict.update(request.json)
            except: raise exceptions.BadRequest('error in request format, cant merge JSON to query string', query_string_dict=query_string_dict, json=request.json)
